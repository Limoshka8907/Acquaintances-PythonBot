...
@dp.message_handler(lambda message: message.text == 'Начать знакомиться' or message.text == '/magic_start',state='*')
async def magic_start(message : types.Message):
	...
	button_language = KeyboardButton('Languages...')
	menu = ReplyKeyboardMarkup()

	if(not db.profile_exists(message.from_user.id)):
			menu.add(button_search,button_create_profile,button_rating_profile, button_language)
	elif(db.profile_exists(message.from_user.id)) :
		#await #print(str(int(db.get_info(str(message.from_user.id))[9])))
		if int(db.get_info(str(message.from_user.id))[8]) < 51:
		...
		menu.add(button_search,button_edit_profile,button_remove_profile,button_rating_profile,button_ranked, button_language)
...
@dp.message_handler(lambda message: message.text == 'Languages...',state='*')
async def set_language(message: types.Message):
	button_exit = KeyboardButton('Выйти❌')
	menu_exit = ReplyKeyboardMarkup()
	menu_exit.add(button_exit)
	l1 = InlineKeyboardButton('English', callback_data='_call_english_lang')
	l2 = InlineKeyboardButton('Russian', callback_data='_call_russian_lang')
	l3 = InlineKeyboardButton('Arabic', callback_data='_call_arabic_lang')

	menu_languages_list = InlineKeyboardMarkup()
	menu_languages_list.add(l1, l2, l3)
	if message.from_user.username != None:
		await message.answer('Выбери один из доступных языков: ', reply_markup=menu_languages_list)
	return
...
async def search_profile(message : types.Message, state: FSMContext):
	...
	try:
		if db.profile_exists(message.from_user.id) == False:
			...
			#print(SearchProfile.city_search)
	except Exception as e:
		await message.answer(cus_ans.random_reapeat_list())
		await state.finish()
		print(e, " - E")
...
async def seach_profile_step2(message: types.Message, state: FSMContext):
	...
	try:
		...
		#print(f"{str(db.get_info(str(message.from_user.id))[8]),str(db.get_info(str(message.from_user.id))[7])} - Чо это " )
		...
		if (bool(len(db.search_profile(str(db.get_info_user(str(message.from_user.id))[4]),str(db.get_info(str(message.from_user.id))[8]),str(db.get_info(str(message.from_user.id))[7]))))):
			try:
				...
				#print("I EXCEPT")
				print(db.edit_zero_profile_status(message.from_user.id), " Edit Zero")
				db.edit_zero_profile_status(message.from_user.id)
				#print("i except 2")

				print(db.search_profile(str(db.get_info_user(str(message.from_user.id))[4]),str(db.get_info(str(message.from_user.id))[8]),str(db.get_info(str(message.from_user.id))[7])), " /# 1")
				print([db.search_profile_status(str(message.from_user.id))[0]][0], " /# 2")
				profile_list = db.search_profile(str(db.get_info_user(str(message.from_user.id))[4]),str(db.get_info(str(message.from_user.id))[8]),str(db.get_info(str(message.from_user.id))[7]))[0]
				profile_id = profile_list[0]
				print(profile_id, " profile Id")

			await state.update_data(last_profile_id=profile_id)
			print(db.search_profile_status(str(message.from_user.id))[0], " - СТАТУС")
			...
			lol_test = str(db.get_info(profile_id))
			...
			#print(f"{name_profile, age_profile} - Human Info")
			city = str(db.get_info_user(message.from_user.id)[5]).title()
...
async def seach_profile_step3(message: types.Message, state: FSMContext):
	...
	try:
		if str(message.text) == '👍':
			...
			try:
				profile_list = db.search_profile(str(db.get_info_user(str(message.from_user.id))[4]),
												 str(db.get_info(str(message.from_user.id))[8]),
												 str(db.get_info(str(message.from_user.id))[7]))[0]
				profile_id = profile_list[0]
			except IndexError:
				db.edit_zero_profile_status(message.from_user.id)
				profile_list = db.search_profile(str(db.get_info_user(str(message.from_user.id))[4]),
				...
				await state.finish()
				await magic_start(message)
			await state.update_data(last_profile_id=profile_id[0])
...
async def backup_step2(message: types.Message, state: FSMContext):
	...
	if str(message.text) == '👍':
		...
		final_text_profile_self = f'Тобой кто то заинтересовался!\n{name_profile_self},{age_profile_self},{city}\n{description_profile_self}\n\nЗнакомиться - @{str(message.from_user.username)}'

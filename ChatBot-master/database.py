...
class dbworker:
	...
	def get_info(self,user_id):
		...
		with self.connection:
			#print(self.cursor.execute("SELECT * FROM `profile_list` WHERE `telegram_id` = ?",(user_id,)).fetchone())
	...
	def search_profile_status(self,user_id):
		...
		try:
			with self.connection:
				#print(self.cursor.execute("SELECT `search_id` FROM `users` WHERE `telegram_id` = ?",(user_id,)).fetchone(), " - RESULT")
				return self.cursor.execute("SELECT `search_id` FROM `users` WHERE `telegram_id` = ?",(user_id,)).fetchone()
		except Exception as e:
			print(e)
	def edit_profile_status(self,user_id,_num):
		'''изменение статуса'''
		with self.connection:
			num = int(_num)
